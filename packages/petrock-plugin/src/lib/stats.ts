...
export function computeCurrentStats(messages: PetMessage[]): PetState {
  let hunger = 0;
  let mood = 0;
  let energy = 0;
  let alive = false;
  ...
  let distressed: Distress | null = null;
  ...
  for (const msg of messages) {
    ...
    switch (msg.action) {
      ...
      case "distressed":
        distressed = {
          reason: (msg.reason as Distress["reason"]) ?? "low_hunger",
...
export function isDead(state: PetState): boolean {
  return state.hunger <= 0 && state.mood <= 0;
}

export function isGraceWindowExpired(state: PetState): boolean {
  if (!state.distressed) return false;
